function generateNote(newNote)
{
	...
	else
	{
		...
		newNote.loadGraphic(Paths.image("flipHUD/skins/pixel/FLIPNOTE_assets", 'data/notetypes'), true, 17, 17);
		newNote.animation.add(Receptor.colors[newNote.noteData] + 'Scroll', [pixelData[newNote.noteData]], 12);
		newNote.animation.play(Receptor.colors[newNote.noteData] + 'Scroll');
...
function generateSplash(splashNote, noteData)
{
	...
	{
		splashNote.frames = Paths.getSparrowAtlas('FLIPnoteSplashes', 'data/notetypes/flipHUD/skins/pixel');
...
function generateSustain(newNote)
{
	...
	else
	{
		...
		newNote.loadGraphic(Paths.image("flipHUD/skins/pixel/HOLD_assets", 'data/notetypes'), true, 7, 6);
		newNote.animation.add(Receptor.colors[newNote.noteData] + 'holdend', [pixelData[newNote.noteData]]);
		newNote.animation.add(Receptor.colors[newNote.noteData] + 'hold', [pixelData[newNote.noteData] - 4]);
		newNote.animation.play(Receptor.colors[newNote.noteData] + 'holdend');
...
function onHit(newNote)
{
	if (!Init.trueSettings.get('Disable Mechanics'))
	{
		...
		else
		{
			...
			hudTween = FlxTween.tween(PlayState.camHUD, {angle: 190}, 1, {ease: FlxEase.circInOut,
				onComplete: function(twn:FlxTween)
				{
					hudTween = FlxTween.tween(PlayState.camHUD, {angle: PlayState.camHUD.angle = 190}, 10, {ease: FlxEase.circInOut,
						onComplete: function(twn:FlxTween)
						{
							hudTween = null;
						}
			...
			{
				noteTween = FlxTween.tween(i, {angle: 190}, 1, {ease: FlxEase.circInOut,
					onComplete: function(twn:FlxTween)
					{
						noteTween = FlxTween.tween(i, {angle: i.angle = 190}, 10, {ease: FlxEase.circInOut,
							onComplete: function(twn:FlxTween)
							{
								noteTween = null;
							}
