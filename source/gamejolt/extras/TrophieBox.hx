...
class TrophieBox extends FlxUIGroup
{
    ...
    public function new(title:String, desc:String, date:String, x:Float = 0, y:Float = 0, ?bg:String)
    {
        super(x, y);
        ...
        var daPath:String = Std.string(Paths.image('trophies/${title.toLowerCase().replace(' ', '-')}'));

        if (FileSystem.exists(daPath)) daImage = new FlxSprite().loadGraphic(daPath);
