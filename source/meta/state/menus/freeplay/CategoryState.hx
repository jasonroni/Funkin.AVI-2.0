...
class CategoryState extends MusicBeat.MusicBeatState {
    ...
    override function create(){
		...
		{
            freeplayCats = ['Legacy', 'Episodes', 'Extras'];
		} else if(FPClientPrefs.muckneyLock == "completed") {
			freeplayCats = ['Legacy', '???', 'Extras'];
			} else {
			 freeplayCats = ['Legacy', '???', '???'];
			}

        BG = new FlxSprite().loadGraphic(Paths.menuImage('menuDesat'));
		BG.updateHitbox();
		BG.screenCenter();
		add(BG);

        #if DISCORD_RPC
		Discord.changePresence('CATEGORY MENU', 'Main Menu');
		#end
    ...
    override public function update(elapsed:Float){
		...
		if (controls.BACK) {
			...
			{
				Main.switchState(this, new VoidState());
			}else{
				FlxG.sound.play(Paths.sound('cancelMenu'));
        ...
        if (controls.ACCEPT){
            switch(curSelected){
				case 0:
					//Main.switchState(this, new LegacyState());
                case 1:
				if(FPClientPrefs.episode1FPLock == 'unlocked')
				{
					Main.switchState(this, new MainState());
				}else{
					FlxG.sound.play(Paths.sound('cancelMenu'));
					noFreeplay.alpha = 1;
                ...
                case 2:
					...
					if(FPClientPrefs.episode1FPLock == 'unlocked' || FPClientPrefs.muckneyLock == "completed")
						{
							Main.switchState(this, new ExtrasState());
						}else{
							FlxG.sound.play(Paths.sound('cancelMenu'));
							noFreeplay.alpha = 1;
							FlxTween.tween(noFreeplay, {alpha: 0}, 1.5, {ease: FlxEase.quadIn, startDelay: 2});
